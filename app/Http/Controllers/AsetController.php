...
class AsetController extends Controller
{
    ...
    {
        abort_unless($request->user()->hasPermission('manage-aset'), 403);

        $validated = $this->validateAset($request);
        $validated['rt_id'] = $this->resolveRtId($request->user());
    ...
    {
        $this->authorizeSameRt($aset, Auth::user());
        $aset->load('rt');
    ...
    {
        abort_unless($user->hasPermission('manage-aset'), 403);
        $this->authorizeSameRt($aset, $user);
    }
