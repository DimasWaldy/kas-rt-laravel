...
<div class="mx-auto max-w-4xl space-y-6">
    ...
    <form method="POST" action="{{ route('aset.store') }}" enctype="multipart/form-data" class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8" x-data="{ preview: null }">
        @csrf
        <div>
            <p class="text-sm font-semibold text-emerald-700">Inventaris RT</p>
            <h1 class="text-2xl font-black text-slate-900">Tambah Aset</h1>
            <p class="mt-1 text-sm text-slate-500">Masukkan data barang yang dimiliki RT.</p>
        </div>
        ...
        <div class="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            <a href="{{ route('aset.index') }}" class="rounded-xl border border-slate-200 px-5 py-3 text-center text-sm font-bold text-slate-600 hover:bg-slate-50">Batal</a>
