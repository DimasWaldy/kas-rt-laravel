...
@section('title', ($filters['scope'] ?? 'rt') === 'rw' ? 'Inventaris Aset RW' : 'Inventaris Aset RT')
...
@php
    ...
    $isRw = ($filters['scope'] ?? 'rt') === 'rw';
    $indexRoute = $isRw ? 'aset-rw.index' : 'aset.index';
    $createRoute = $isRw ? 'aset-rw.create' : 'aset.create';
...
<div class="mx-auto max-w-7xl space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <p class="text-sm font-semibold text-emerald-700">{{ $isRw ? 'Fasilitas bersama lintas RT' : 'Inventaris lingkungan RT' }}</p>
            <h1 class="text-2xl font-black text-slate-900">{{ $isRw ? 'Inventaris Aset RW' : 'Inventaris Aset RT' }}</h1>
            <p class="mt-1 text-sm text-slate-500">{{ $isRw ? 'Kelola aset RW seperti balai, lapangan, dan fasilitas bersama.' : 'Kelola barang RT dan ajukan peminjaman aset yang tersedia.' }}</p>
        </div>
    ...
    <form method="GET" action="{{ route($indexRoute) }}" class="space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div>
    ...
    @if($asets->isEmpty())
        <div class="rounded-3xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center">
            ...
            <p class="mt-1 text-sm text-slate-500">{{ $isRw ? 'Aset RW yang sudah didata akan tampil di sini.' : 'Aset RT yang sudah didata akan tampil di sini.' }}</p>
        </div>
    @else
                ...
                <article class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
                    ...
                    <div class="p-5">
                        ...
                        <div class="mt-4 space-y-2 text-sm text-slate-600">
                            ...
                            <p class="flex items-center gap-2"><i class="fa-solid fa-building-user w-4 text-emerald-600"></i><span>{{ $aset->owner_name }}</span></p>
                        </div>

                        <div class="mt-5 grid gap-2 sm:grid-cols-2">
                            @if($canPinjam && $aset->jumlah_tersedia > 0 && $aset->is_active && $aset->kondisi !== 'rusak_berat')
                                <a href="{{ route($pinjamCreateRoute, ['aset_id' => $aset->id]) }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-700 px-4 py-3 text-sm font-bold text-white hover:bg-emerald-800">
                                    Pinjam
                                </a>
                            @endif
                            <a href="{{ route($showRoute, $aset) }}" class="inline-flex items-center justify-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-bold text-emerald-800 hover:bg-emerald-100">
