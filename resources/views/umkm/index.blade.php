    ...
    @else
        <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            @foreach($umkms as $umkm)
                @php($isBuka = $umkm->isBukaSekarang())
                <article class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg" x-data="{ rejectOpen: {{ $errors->has('catatan_pengurus') && old('umkm_id') == $umkm->id ? 'true' : 'false' }} }">
                    ...
                    <div class="p-5">
                        ...
                        <div class="mt-4 flex items-center justify-between gap-3 rounded-2xl bg-slate-50 px-3 py-2.5">
                            <p class="min-w-0 truncate text-xs font-semibold text-slate-600"><i class="fa-regular fa-clock mr-1 text-emerald-600"></i>{{ $umkm->jam_operasional_ringkas }}</p>
                            @if($umkm->status === 'approved')
                                <span class="shrink-0 rounded-full px-2.5 py-1 text-[10px] font-black {{ $isBuka ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600' }}">
                                    {{ $isBuka ? 'Buka' : 'Tutup' }}
                                </span>
                            @endif
                        </div>
