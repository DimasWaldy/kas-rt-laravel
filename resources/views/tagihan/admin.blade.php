    ...
    <div class="bg-white rounded-3xl shadow-sm p-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-slate-600 mt-2">Lihat, buat, dan kelola tagihan iuran warga.</p>
            </div>
    ...
    <div class="bg-white rounded-3xl shadow-sm p-6 border border-slate-200 overflow-auto">
        <table class="min-w-full divide-y divide-slate-200 text-sm">
            <thead class="bg-slate-50">
                <tr>
                    <th class="px-4 py-3 text-left font-semibold text-slate-600">Warga</th>
                    ...
                    <th class="px-4 py-3 text-left font-semibold text-slate-600">Bukti</th>
            ...
            <tbody class="divide-y divide-slate-200">
                @forelse($tagihans as $tagihan)
                    <tr>
                        <td class="px-4 py-4 whitespace-nowrap text-slate-700">{{ $tagihan->user->name }}</td>
                        ...
                        <td class="px-4 py-4 whitespace-nowrap text-slate-700">
                            @if($tagihan->bukti)
                                <a href="{{ Storage::url($tagihan->bukti) }}" target="_blank" class="text-blue-600 hover:underline">Lihat</a>
                            @else
                                -
                            @endif
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="flex flex-col gap-2">
                                <form action="{{ route('tagihan.confirm') }}" method="POST" class="flex gap-2">
                                    ...
                                    <button type="submit" class="rounded-lg bg-blue-600 px-3 py-1 text-xs font-semibold text-white hover:bg-blue-700 transition whitespace-nowrap">Simpan</button>
                                </form>
